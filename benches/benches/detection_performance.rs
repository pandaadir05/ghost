...
fn bench_detection_engine(c: &mut Criterion) {
    ...
    c.bench_function("process_analysis_small", |b| {
        ...
        b.iter(|| {
            engine.analyze_process(black_box(&process), black_box(&regions), None)
        })
    });
...
fn bench_shellcode_detection(c: &mut Criterion) {
    ...
    c.bench_function("shellcode_scan_1kb", |b| {
        ...
        b.iter(|| {
            detector.scan_memory_region(black_box(&data), black_box(0x10000000))
        })
    });
    ...
    c.bench_function("shellcode_scan_with_patterns", |b| {
        ...
        data[100] = 0x4D; data[101] = 0x5A; // MZ header
        data[200] = 0xFC; data[201] = 0x48; // Meterpreter pattern
        data[300] = 0x64; data[301] = 0x8B; // PEB access
...
fn bench_memory_operations(c: &mut Criterion) {
    c.bench_function("memory_region_creation", |b| {
        b.iter(|| {
            create_memory_regions(black_box(100))
        })
    });
