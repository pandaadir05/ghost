...
class BinaryTransfer:
    ...
    def calculate_checksum(self, binary_data: bytes, algorithm: str = "sha256") -> str:
        ...
        try:
            # SECURITY NOTE: MD5 and SHA1 are cryptographically broken
            # Only use them for legacy compatibility when absolutely necessary
            # SHA256 or better should be used for security-sensitive applications
            if algorithm == "md5":
                logger.warning("MD5 is cryptographically broken - use SHA256 or better")
                hasher = hashlib.md5()
            elif algorithm == "sha1":
                logger.warning("SHA1 is cryptographically weak - use SHA256 or better")
                hasher = hashlib.sha1()
                ...
                logger.warning(f"Unknown algorithm {algorithm}, defaulting to SHA256")
                hasher = hashlib.sha256()
